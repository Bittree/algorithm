...
var WordsMap map[string]Empty

func containes(target string) bool {
	_, ok := WordsMap[target]
	if ok {
		return true
	} else {
		return false
...
func nextWords(word string, WordList []string) []string {
	ret := []string{}
	for i, ch := range word {
		for c := 'a'; c <= 'z'; c++ {
			if ch != c {
				var builder strings.Builder
				builder.WriteString(word[:i])
				builder.WriteRune(c)
				builder.WriteString(word[i+1:])
				newWord := builder.String()
				if containes(newWord) {
					ret = append(ret, newWord)
				}
			}
		}
	}
...
func bfs(beginWord string, endWord string, wordList []string) [][]string {
	records := []string{beginWord}
	ret := [][]string{records}
	ans := [][]string{}
	visitedWords := make(map[string]Empty)
	visitedWords[beginWord] = empty
	found := false
	var size int
	for {
		size = len(ret)
		...
		thisLayerVisitedWords := make(map[string]Empty)
		for i := 0; i < size; i++ {
			curPath := ret[i]
			target := curPath[len(curPath)-1]
			childs := nextWords(target, wordList)
			for _, child := range childs {
				_, ok := visitedWords[child]
				if !ok {
					result := make([]string, len(curPath)+1)
					copy(result, curPath)
					result[len(curPath)] = child
					ret = append(ret, result)
					thisLayerVisitedWords[child] = empty
					if child == endWord {
						found = true
						newArr := make([]string, len(result))
						copy(newArr, result)
		...
		if found {
			...
			for k, v := range thisLayerVisitedWords {
				visitedWords[k] = v
			}
		}
	}
...
func findLadders(beginWord string, endWord string, wordList []string) [][]string {
	WordsMap = make(map[string]Empty)
	for _, v := range wordList {
		WordsMap[v] = empty
	}
	if containes(endWord) {
...
func main() {
	...
	// fmt.Println("beginWord: ", beginWord)
